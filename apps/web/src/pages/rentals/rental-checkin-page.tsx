...
import { checkinRental, getRentalById } from '@/services/rentals-service';

export function RentalCheckinPage() {
  ...
  const checkinMutation = useMutation({
    mutationFn: () => checkinRental(safeRentalId),
    onSuccess: async () => {
      showSuccessToast('Cart checked in. Rental is now completed.');
  ...
  if (!safeRentalId) {
    return (
      <StaffPageLayout title="Check In" subtitle="Invalid rental id." currentPath="/rentals">
        <PageError message="Rental id is missing from the route." />
      </StaffPageLayout>
    );
  }

  const isNotActive = rental && rental.status !== RentalStatus.active;

  return (
    <StaffPageLayout
      title="Check In Cart"
      subtitle="Confirm check-in to record the actual return time and calculate the final amount."
      currentPath="/rentals"
      headingSlot={
            ...
            <CardContent className="space-y-4">
              ...
              <div className="border-t border-border pt-4">
                <div className="grid gap-4 md:grid-cols-4">
                  ...
                  <div>
                    <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Expected Return</p>
                    <p className="mt-0.5 text-sm text-foreground">{formatDate(rental.endDate)}</p>
                  </div>
                  <div>
                    <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Expected Total</p>
                    <p className="mt-0.5 text-sm font-semibold text-foreground">{formatCurrency(rental.totalAmount)}</p>
                  </div>
                </div>
              </div>

              <div className="rounded-lg border border-border bg-[var(--color-background-subtle)] px-4 py-3">
                <p className="text-sm text-muted-foreground">
                  Confirming check-in will record the actual return time as{' '}
                  <strong className="text-foreground">now</strong> and calculate the final amount
          ...
          {isNotActive ? (
            <Card className="border border-border bg-background shadow-sm">
              <CardContent className="py-5">
                <p className="text-sm text-muted-foreground">
                  This rental is in <strong>{formatStatusLabel(rental.status)}</strong> status and
