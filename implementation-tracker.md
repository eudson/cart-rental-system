...
- 2026-04-12: Improved API test suite — 5-task improvement pass. (1) Stabilized `workspace-scripts.test.ts`: replaced fragile exact-value assertion on `scripts.dev` with behavioral `assert.ok`. (2) Added rental concurrency test: fires two parallel `POST /rentals` for the same cart and window, asserts only one 201 and exactly one DB record. (3) Extracted shared helpers to `apps/api/test/helpers.ts` (`setupTestApp`, `loginAsStaff`, `loginAsCustomer`) and migrated 5 test files. (4) Added 10 boundary/validation tests across `rentals.test.ts`, `carts.test.ts`, and `customers.test.ts`: missing required fields, invalid enum values, org-boundary GET/PATCH isolation, and pageSize >100. (5) Added `apps/api/test/business-logic.test.ts` with 17 fast unit tests covering `buildPaginationMeta`, `calculatePaginationOffset`, `VALID_CART_TRANSITIONS`, and outstanding-balance formula — no app boot required. Total: 141/141 passing (was 113/114 with 1 failure before).
